...
- Listing cards render through =@jaydixit/astro-utils/components/Image.astro= which auto-generates responsive markup for Cloudinary URLs. Cloudinary helpers in =src/lib/cloudinary.ts= build the delivery URLs/sizes.
